...
class AdminMessage(models.Model):
    ...
    title = models.CharField(max_length=settings.DEFAULT_CHAR_FIELD_MAX_LENGHT, verbose_name='Навзвание')
    text = models.TextField(verbose_name='Текст сообщения')
    key = models.CharField(
        max_length=settings.DEFAULT_CHAR_FIELD_MAX_LENGHT, verbose_name='Ключ, по которому сообщение вызывается в коде'
    )
...
class Message(models.Model):
    ...
    mailing = models.ForeignKey(
        Mailing, related_name='messages', on_delete=models.PROTECT, blank=True, null=True
    )
