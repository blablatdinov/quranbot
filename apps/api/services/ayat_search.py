...
class AyatSearcher:
    ...
    def get_ayat_by_sura_ayat(self) -> Ayat:
        ...
        ayats_in_sura = Ayat.objects.filter(sura__number=sura_number)  # TODO разнести функцию, не читаемый код
        for ayat in ayats_in_sura:
            if '-' in str(ayat):  # Для кейсов типа 2:1-5
                self._hyphen_cases(ayat)
            elif ',' in str(ayat):  # Для кейсов типа 3:5,6
                name = [int(x) for x in str(ayat).split(':')[1].split(',')]
                if ayat_number in name:
                    return ayat
            elif int(ayat.ayat) == ayat_num:  # Для кейсов, когда название можно перевести в чиcло
                return ayat
