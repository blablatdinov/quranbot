...
@pytest.fixture()
def mock(http_mock, nominatium_response, geonames_response):
    http_mock.get(
        'https://nominatim.openstreetmap.org/search?q=%D0%92%D0%BB%D0%B0%D0%B4%D0%B8%D0%B2%D0%BE%D1%81%D1%82%D0%BE'
        '%D0%BA&format=json&limit=1',
        text=nominatium_response,
    )
    http_mock.get(
        'http://api.geonames.org/timezoneJSON?lat=43.1150678&lng=131.8855768&username=blablatdinov',
        text=geonames_response,
    )


@pytest.mark.freeze_time('2021-03-20 01:00:00')
def test_name(vladivostok_prayers, subscriber, mock):  # FIXME naming
